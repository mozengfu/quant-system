#!/usr/bin/env python3
"""
ML选股模型训练 V6.7 — 修复泄漏 + 调参 + 精简特征

相较 V6.6 的主要变更:
  - 修复大宗交易特征泄漏（按日期对齐，而非全序列取最后值）
  - 修复业绩预告特征泄漏（merge_asof 替代 iloc[-1]）
  - amount_est 提前计算，确保融资融券等特征也能用到
  - 精简特征集，移除北向资金等无效特征
  - 超参数调整：num_leaves=64, lr=0.01, min_child_samples=200, lambda_l2=0.01
  - Label 离散化改为 10 bins，label_gain = range(10)
  - 固定 120 天滚动窗口 walk-forward
"""
...
MODEL_PATH = os.path.join(DATA_DIR, 'ml_stock_model_v6_7.pkl')
FEATURE_CONFIG_PATH = os.path.join(DATA_DIR, 'feature_config_v6_7.json')
...
def build_features(daily, moneyflow, index_data, fundamentals, stock_info, alpha_signals,
                   ...
                   min_date, max_date):
    """
    V6.7 特征构建 — 修复泄漏 + 精简特征集
    """
    logger.info("构建特征 V6.7...")
    ...
    for ts_code, group in daily.groupby('ts_code'):
        ...
        # ====== V6.7 修复: amount_est 提前计算（确保融资融券等特征也能用到） ======
        ...
        # ====== V6.7 修复: 业绩预告特征（merge_asof 防泄漏） ======
        fc_stock = stock_forecast[stock_forecast['ts_code'] == ts_code].copy()
        if not fc_stock.empty and len(g) > 0:
            fc_stock = fc_stock.sort_values('report_date').rename(
                columns={'report_date': 'trade_date'}
            )
            fc_stock['trade_date'] = pd.to_datetime(fc_stock['trade_date'])
            # 统一 dtype 避免 merge_asof 报错
            g_temp = g[['trade_date']].reset_index(drop=True).copy()
            if not g_temp.empty:
                fc_stock['trade_date'] = fc_stock['trade_date'].astype(g_temp['trade_date'].dtype)
                fc_stock = fc_stock[fc_stock['trade_date'] <= g['trade_date'].max()]

                # merge_asof: 对每个交易日，向前匹配最近一期的业绩预告
                ...
                merged['forecast_type_code'] = merged['forecast_type'].map(type_map).fillna(0)
                merged['forecast_is_positive'] = merged['forecast_type'].isin(
                    ['预增', '扭亏', '略增', '续盈']
                ).astype(int)
                merged['forecast_net_profit_max'] = pd.to_numeric(merged['net_profit_max'], errors='coerce').fillna(0)
                merged['forecast_days_since'] = (merged['trade_date'] - merged['fc_date']).dt.days.fillna(30)
    ...
    logger.info(f"构建完成: {len(result):,} 样本, {result['ts_code'].nunique()} 股, 约 100 特征")
...
def walk_forward_train(df, feature_cols):
    ...
    """
    logger.info("Walk-Forward 滚动验证 (expanding window)...")
    ...
    for fold in range(5):
        ...
        train_df['label_rank'] = train_df.groupby('trade_date')['alpha_5d'].transform(
            lambda x: safe_qcut(x, 10)  # 10 bins
        )
        ...
        params = {
            ...
            'label_gain': list(range(10)),  # 10 bins
            'boosting_type': 'gbdt',
            'num_leaves': 64,
            'learning_rate': 0.01,
