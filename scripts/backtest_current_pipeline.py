...
from ml_predict import _load_best_model, _build_features_for_stocks_v8_6, _ensemble_predict
...
def get_top_vol_yesterday(conn, date_str, n=TOP_POOL):
    ...
    df = pd.read_sql(f"""
        SELECT ts_code, amount FROM daily_price WHERE trade_date = '{prev_date}'
...
def forward_return_clean(conn, code, buy_date_str, hold=5):
    df = pd.read_sql(f"""
        SELECT trade_date, pct_chg FROM daily_price WHERE ts_code = '{code}' AND trade_date > '{buy_date_str}'
        ORDER BY trade_date LIMIT {hold}""", conn)
...
def filter_pipeline(conn, codes, scores, buy_date, feat_df):
    ...
    for c in codes:
        ...
        passed.append({
            ...
            'pct_chg': 0, 'volume_ratio': 0, 'rps_20': 0, 'turnover_rate': 0,
            'close': 0, 'high_52w': 0, 'low_52w': 0, 'main_net': 0,
        })
    ...
    try:
        ...
        if hm_codes:
            ...
            # 连板
            cur.execute(f"SELECT ts_code, COALESCE(MAX(last_board),0) FROM zt_pool WHERE ts_code IN ({ph}) AND trade_date >= DATE_SUB(%s, INTERVAL 60 DAY) AND last_board>0 GROUP BY ts_code", (*hm_codes, display_date))
            board_map = {r[0]: int(r[1] or 0) for r in cur.fetchall()}
            # 封单萎缩
            cur.execute(f"SELECT ts_code, trade_date, seal_amount FROM zt_pool WHERE ts_code IN ({ph}) AND trade_date>=DATE_SUB(%s, INTERVAL 30 DAY) ORDER BY ts_code,trade_date DESC", (*hm_codes, display_date))
            seal_map = {}
            for r in cur.fetchall():
                seal_map.setdefault(r[0], []).append(float(r[2] or 0))
            # 15日内涨停+跌停
            cur.execute(f"SELECT ts_code, SUM(CASE WHEN pct_chg>=9.5 THEN 1 ELSE 0 END), SUM(CASE WHEN pct_chg<=-9.5 THEN 1 ELSE 0 END), MIN(CASE WHEN pct_chg>=9.5 THEN trade_date END), MAX(CASE WHEN pct_chg<=-9.5 THEN trade_date END) FROM daily_price WHERE ts_code IN ({ph}) AND trade_date>=DATE_SUB(%s, INTERVAL 15 DAY) AND trade_date<=%s GROUP BY ts_code", (*hm_codes, display_date, display_date))
            ud_map = {r[0]: {'up': r[1] or 0, 'down': r[2] or 0, 'first_up': r[3], 'last_down': r[4]} for r in cur.fetchall()}
            # 10日换手
            cur.execute(f"SELECT ts_code, AVG(turnover_rate), MAX(turnover_rate) FROM daily_price WHERE ts_code IN ({ph}) AND trade_date>=DATE_SUB(%s, INTERVAL 10 DAY) AND trade_date<=%s GROUP BY ts_code", (*hm_codes, display_date, display_date))
            tr_map = {r[0]: {'avg': float(r[1] or 0), 'max': float(r[2] or 0)} for r in cur.fetchall()}
            # 10日主力
            cur.execute(f"SELECT ts_code, COALESCE(SUM(main_net),0) FROM moneyflow_daily WHERE ts_code IN ({ph}) AND trade_date>=DATE_SUB(%s, INTERVAL 10 DAY) GROUP BY ts_code", (*hm_codes, display_date))
            ...
            for c in passed:
                ...
                avg_tr = tr.get('avg', 0)
                if avg_tr > 20: sc += 15
                elif avg_tr > 15 and tr.get('max', 0) > 25: sc += 10
                if main_map.get(tc, 0) < -30000000: sc += 15
                if sc >= 40:
                    hm_excluded.add(tc)
            if hm_excluded:
    ...
    try:
        ...
        if e_codes:
            ...
            cur.execute(f"SELECT e.ts_code, e.net_profit_yoy FROM earnings_report e WHERE e.ts_code IN ({ph2}) AND e.report_date=(SELECT MAX(e2.report_date) FROM earnings_report e2 WHERE e2.ts_code=e.ts_code)", e_codes)
...
def main():
    ...
    stats = {'total': len(sample_dates), 'has_pos': 0, 'after_risk': 0, 'after_youzi': 0, 'after_earnings': 0, 'has_reco': 0}
    ...
    for di, buy_date in enumerate(sample_dates):
        ...
        if len(vol_codes) < 50: continue

        try:
            feat = _build_features_for_stocks_v8_6(conn, vol_codes, as_of_date=buy_date)
        except Exception:
            continue
        if feat is None or feat.empty or len(feat) < 20: continue
    ...
    for label, display in [('raw_ml', '① 纯ML(无过滤)'), ('ml_pos', '② ML正分过滤'), ('full_pipeline', '③ 完整管线(正分+风控+游资+业绩+行业)')]:
        store = results[label]
    ...
    # 差异分析
    for l1, l2, name in [('raw_ml', 'ml_pos', '②比①'),
        ...
        if s1 and s2:
            ...
            if common:
                ...
                print(f"  {name}: 共同{len(common)}天 | 均值差{np.mean(diffs):+.2f}% | 改进{(np.array(diffs)>0).sum()}/{len(common)}")

    output = {
        'model': version,
        'params': {'start': START_DATE, 'end': END_DATE, 'pool': TOP_POOL,
                   'interval': SAMPLE_INTERVAL, 'top_n': TOP_N, 'hold_days': HOLD_DAYS},
        'stats': stats,
