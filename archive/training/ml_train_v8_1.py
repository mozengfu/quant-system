#!/usr/bin/env python3
"""
ML选股模型训练 V8.0 — V6.2 + V6.3 精选特征（去零贡献 + 去冗余）

相比 V6.4 精简了零贡献特征:
  - 移除 ln_mv/pe_ttm/pb/ln_circ_mv/is_st/ln_list_age (重要性为 0)
  - 移除 net_margin, zt_open_count_30d (重要性为 0)
  - 移除 dragon_has_institution_30d (重要性 < 1)
  - 保留其余全部 105 个有效特征

支持 --max_date YYYY-MM-DD 参数，用于训练样本外模型回测。
"""
...
oos_model_path = os.path.join(DATA_DIR, 'ml_stock_model_v8_1.pkl')
FEATURE_CONFIG_PATH = os.path.join(DATA_DIR, 'feature_config_v8_1.json')
...
def load_data(max_date=None):
    """
    加载训练数据
    max_date: 可选，限制最大日期（用于样本外训练）
    """
    if max_date:
        logger.info(f"加载数据（截止 {max_date}）...")
        max_dt = datetime.strptime(max_date, '%Y-%m-%d')
        min_dt = max_dt - timedelta(days=600)
        trade_bound = f"trade_date >= '{min_dt.date()}' AND trade_date <= '{max_dt.date()}'"
        end_bound = f"end_date >= '{min_dt.date()}' AND end_date <= '{max_dt.date()}'"
        max_date_sql = f"'{max_date}'"
    else:
        logger.info("加载最近 600 个交易日数据...")
        max_expr = "(SELECT MAX(trade_date) FROM daily_price)"
        trade_bound = f"trade_date >= {max_expr} - INTERVAL 600 DAY"
        end_bound = f"end_date >= {max_expr} - INTERVAL 600 DAY"
        max_date_sql = max_expr
    ...
    regime_df = pd.read_sql("""
        SELECT trade_date, score as regime_score, prob_bull, prob_bear, prob_panic,
               prob_range, prob_overheated, sh_trend, market_breadth,
               volatility, volume_trend, momentum_breadth
        FROM market_regime_daily ORDER BY trade_date
    """, conn)
    for c in regime_df.columns:
        if c != 'trade_date':
            regime_df[c] = regime_df[c].fillna(0).astype(float)
    logger.info(f"市场状态: {len(regime_df)} 天")
...
def build_features(daily, moneyflow, index_data, fundamentals, stock_info, alpha_signals,
    ...
    for ts_code, group in daily.groupby('ts_code'):
        ...
        g["vol_20d_for_label"] = g["pct_chg"].shift(1).rolling(20).std() * np.sqrt(5)
        g["target_5d_adj"] = g["target_5d"] / (g["vol_20d_for_label"] + 0.01)
    ...
    if not regime_df.empty:
        ...
        result = result.merge(regime_df, on='trade_date', how='left')
        for c in regime_df.columns:
            if c != 'trade_date':
    ...
    # V8.1: 确保市场状态特征在特征列中
    regime_feature_cols = [
        'regime_score', 'prob_bull', 'prob_bear', 'prob_panic',
        'prob_range', 'prob_overheated', 'sh_trend', 'market_breadth',
        'volatility', 'volume_trend', 'momentum_breadth',
    ]
...
def main():
    ...
    # 解析参数
    import argparse
    parser = argparse.ArgumentParser(description='ML选股模型训练 V8.1')
    parser.add_argument('--max_date', type=str, default=None,
                        help='训练数据截止日期 (YYYY-MM-DD)，用于训练样本外模型回测')
    parser.add_argument('--output', type=str, default=None,
                        help='模型输出路径，默认 data/ml_stock_model_v8_1.pkl')
    args = parser.parse_args()

    # 使用自定义输出路径
    oos_model_path = args.output or oos_model_path
    ...
    (daily, moneyflow, index_data, fundamentals, stock_info, alpha_signals,
     ...
     block_trade, stock_forecast,
     regime_df,
     min_date, max_date) = load_data(max_date=args.max_date)
    ...
    logger.info(f"模型保存: {oos_model_path}")
