#!/usr/bin/env python3
"""
ML选股模型训练 V9.0 — V6.5 超参数回归 + 波动率截断 + 去 rank 特征 + 800天窗口

相比 V8.0 核心改进:
  - 超参数回退到 V6.5（num_leaves=31, lr=0.05, min_child=500, 5 bins, 300轮）
  - 波动率分母截断：clip(lower=0.02) 避免低 vol 股票 label 异常放大
  - 移除所有 *_rank 横截面排名特征（125→~93 特征）
  - 数据窗口 600→800 天，增加训练样本和 WF 稳定性
  - 新增 lambda_l1=0.1 L1 正则
"""
...
MODEL_PATH = os.path.join(DATA_DIR, 'ml_stock_model_v9_0.pkl')
FEATURE_CONFIG_PATH = os.path.join(DATA_DIR, 'feature_config_v9_0.json')
...
def load_data():
    logger.info("加载最近 1300 日（约 800 交易日）数据...")
    conn = get_db()
    ...
    daily = pd.read_sql("""
        ...
        FROM daily_price
        WHERE trade_date >= (SELECT MAX(trade_date) FROM daily_price) - INTERVAL 1300 DAY
    """, conn)
    ...
    index_data = pd.read_sql("""
        ...
        AND trade_date >= (SELECT MAX(trade_date) FROM daily_price) - INTERVAL 1300 DAY
    """, conn)
    ...
    holder_change = pd.read_sql("""
        ...
        FROM holder_change
        WHERE end_date >= (SELECT MAX(trade_date) FROM daily_price) - INTERVAL 1300 DAY
    """, conn)
    ...
    train_start = dates[-700] if len(dates) > 700 else dates[0]
...
def build_features(daily, moneyflow, index_data, fundamentals, stock_info, alpha_signals,
    ...
    for ts_code, group in daily.groupby('ts_code'):
        ...
        # --- V9.0 回归目标：简单 5 日收益（与 V6.5 一致，波动率调整引入噪声） ---
    ...
    # V9.0: 移除横截面排名特征（与原始特征高度共线，导致过拟合）
    # logger.info("添加横截面排名特征...")
    # rank_features = [...]  # REMOVED
...
def walk_forward_train(df, feature_cols):
    ...
    # V9.0: 5-fold expanding window (与 V6.5 一致)
    n_folds = 5
    window_size = len(dates) // (n_folds + 2)
    ...
    for fold in range(n_folds):
        ...
        # LambdaRank 离散化（V9.0: 5 bins 粗粒度，不易 memorize 噪声）
...
def train_ensemble(df, feature_cols, n_models=5):
    ...
    for i in range(n_models):
        ...
        model = lgb.train(
            params, base_ds, num_boost_round=500,
...
def main():
    ...
    bundle = {
        ...
        'label': 'v9_0_v65_hyperparams_vol_capped_800day',
