#!/usr/bin/env python3
"""
ML选股模型训练 V8.5 — v8.3 + alpha_5d Winsorize (消除 IPO/退市极端值)

修复:
  - alpha_5d Winsorize 1%-99% 分位
  - 标签: 原始5日收益（非波动率调整）
...
def build_features(daily, moneyflow, index_data, fundamentals, stock_info, alpha_signals,
    ...
    # === V8.5: Winsorize label 消除极端值 (IPO/退市) ===
    logger.info("Winsorize alpha_5d 消除极端值...")
    p_low = result['alpha_5d'].quantile(0.01)
    p_high = result['alpha_5d'].quantile(0.99)
    result['alpha_5d'] = result['alpha_5d'].clip(p_low, p_high)

    alpha_mean = result['alpha_5d'].mean() * 100
    alpha_std = result['alpha_5d'].std() * 100
    logger.info(f"alpha_5d 分布 (Winsorized 1%%-99%%): 均值={alpha_mean:.2f}%, std={alpha_std:.2f}%")
