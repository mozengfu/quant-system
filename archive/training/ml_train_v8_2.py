#!/usr/bin/env python3
"""
ML选股模型训练 V8.2 — V8.1 + 10日标签 + 800天窗口 + 选择性排名特征 + 超参数优化

V8.2 改进:
  - 标签: 5日超额收益 → 10日超额收益（降低噪声）
  - 训练窗口: 600 → 800 天（更多样本，覆盖更多周期）
  - 排名特征: 完全移除 → 选择性保留 5 个高 IC 排名
  - 超参数: num_leaves 64→32, lr 0.01→0.005, early_stop 50→100
"""
...
MODEL_PATH = os.path.join(DATA_DIR, 'ml_stock_model_v8_2.pkl')
FEATURE_CONFIG_PATH = os.path.join(DATA_DIR, 'feature_config_v8_2.json')
...
def load_data():
    ...
    daily = pd.read_sql("""
        ...
        FROM daily_price
        WHERE trade_date >= (SELECT MAX(trade_date) FROM daily_price) - INTERVAL 800 DAY
    """, conn)
    ...
    index_data = pd.read_sql("""
        ...
        AND trade_date >= (SELECT MAX(trade_date) FROM daily_price) - INTERVAL 800 DAY
    """, conn)
    ...
    holder_change = pd.read_sql("""
        ...
        FROM holder_change
        WHERE end_date >= (SELECT MAX(trade_date) FROM daily_price) - INTERVAL 800 DAY
    """, conn)
    ...
    min_date = dates[-500] if len(dates) > 500 else dates[0]
    max_date = dates[-1]

    conn.close()

    train_start = dates[-500] if len(dates) > 500 else dates[0]
...
def build_features(daily, moneyflow, index_data, fundamentals, stock_info, alpha_signals,
    ...
    for ts_code, group in daily.groupby('ts_code'):
        ...
        # --- V8.0 回归目标：波动率调整的5日收益 ---
        g["target_10d"] = g["close"].shift(-10) / g["close"] - 1
        g["vol_20d_for_label"] = g["pct_chg"].shift(1).rolling(20).std() * np.sqrt(10)
        g["target_10d_adj"] = g["target_10d"] / (g["vol_20d_for_label"] + 0.01)
    ...
    # === 行业超额收益目标 ===
    logger.info("计算行业超额收益 alpha_10d...")
    ...
    alpha_mean = result['alpha_10d'].mean() * 100
    alpha_std = result['alpha_10d'].std() * 100
    logger.info(f"alpha_10d 分布: 均值={alpha_mean:.2f}%, std={alpha_std:.2f}%")
    ...
    # V8.2: 选择性保留 5 个高 IC 排名特征（其余已移除以防共线）
    logger.info("添加选择性排名特征（5个高IC）...")
    rank_features_v82 = [
        'lg_ratio',      # 大单比率排名（资金流核心）
        'pos_52w',       # 52 周位置排名（动量因子）
        'main_cum5',     # 主力 5 日净流入排名
        'pct_chg',       # 涨跌幅排名
        'rps_20',        # RPS 排名
    ]
    for col in rank_features_v82:
    ...
    logger.info(f"alpha_10d: 均值={y.mean()*100:.2f}%, 中位={np.median(y)*100:.2f}%, "
...
def walk_forward_train(df, feature_cols):
    ...
    for fold in range(5):
        ...
        y_train = df_sorted.loc[train_mask, 'alpha_10d'].values
        X_val = df_sorted.loc[val_mask, feature_cols].values
        y_val = df_sorted.loc[val_mask, 'alpha_10d'].values
        ...
        train_df['label_rank'] = train_df.groupby('trade_date')['alpha_10d'].transform(
            lambda x: safe_qcut(x, 10)
        )
        val_df['label_rank'] = val_df.groupby('trade_date')['alpha_10d'].transform(
            lambda x: safe_qcut(x, 10)
        )
        ...
        model = lgb.train(
            params, td, num_boost_round=3000, valid_sets=[vd],
        ...
        daily_ic = val_df_cv.groupby('trade_date').apply(
            lambda x: spearmanr(x['alpha_10d'], x['pred'])[0], include_groups=False
        )
...
def train_ensemble(df, feature_cols, n_models=5):
    ...
    df_temp['weight'] = np.exp(-0.002 * df_temp['days_ago'])
    sample_weight = df_temp['weight'].values

    # 离散化标签
    df_temp['label_rank'] = df_temp.groupby('trade_date')['alpha_10d'].transform(
    ...
    for i in range(n_models):
        ...
        params['feature_fraction'] = [0.6, 0.65, 0.7, 0.75, 0.8][i]  # 每个模型不同特征采样率

        model = lgb.train(
            params, base_ds, num_boost_round=3000,
    ...
    # 评估集成效果
    final_rank_ic = spearmanr(df_sorted['alpha_10d'].values, ensemble_pred)[0]

    daily_ic = df_sorted.groupby('trade_date').apply(
        lambda x: spearmanr(x['alpha_10d'].values,
...
def main():
    ...
    bundle = {
        ...
        'label': 'v8_2_10d_800day_selective_rank_regularized',
