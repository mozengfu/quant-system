#!/usr/bin/env python3
"""
ML选股模型训练 V8.4 — 全量特征（含idx_*市场相对特征 + v6.5超参数）

修复记录:
  - 恢复 idx_* 指数相对特征（4/29 v6.5 有, IC=0.260, 后被误删）
  - 标签: 原始5日收益（非波动率调整）
  - 超参数: num_leaves=31, lr=0.05, 5 bins, early_stopping=30
  - 特征: v6.5 全部 + v6.2/v6.3/v8.0 新增
"""
...
MODEL_PATH = os.path.join(DATA_DIR, 'ml_stock_model_v8_4.pkl')
FEATURE_CONFIG_PATH = os.path.join(DATA_DIR, 'feature_config_v8_4.json')
...
def build_features(daily, moneyflow, index_data, fundamentals, stock_info, alpha_signals,
    ...
    for ts_code, group in daily.groupby('ts_code'):
        ...
        # === V8.4: 恢复 idx_* 市场相对特征 ===
        g = g.merge(index_data[['trade_date', 'close_price', 'change_pct']].rename(
            columns={'close_price': 'idx_close', 'change_pct': 'idx_chg'}),
            on='trade_date', how='left')
        g['idx_close'] = g['idx_close'].fillna(g['close'])
        g['idx_chg'] = g['idx_chg'].fillna(0)

        g['idx_ma5'] = g['idx_close'].ewm(span=5, adjust=False).mean()
        g['idx_ma20'] = g['idx_close'].ewm(span=20, adjust=False).mean()
        g['idx_ma60'] = g['idx_close'].ewm(span=60, adjust=False).mean()

        g['idx_ma5_ratio'] = g['close'] / g['idx_ma5'].replace(0, np.nan)
        g['idx_ma20_ratio'] = g['close'] / g['idx_ma60'].replace(0, np.nan)
        g['idx_ma60_ratio'] = g['close'] / g['idx_ma60'].replace(0, np.nan)

        g['idx_pct_5d'] = g['close'].shift(1) / g['idx_close'].shift(5).replace(0, np.nan) - 1
        g['idx_pct_20d'] = g['close'].shift(1) / g['idx_close'].shift(21).replace(0, np.nan) - 1
        g['idx_pct_5d'] = g['idx_pct_5d'].shift(1)
        g['idx_pct_20d'] = g['idx_pct_20d'].shift(1)

        g['idx_vol_10d'] = g['idx_chg'].shift(1).rolling(10).std()
        g['idx_vol_20d'] = g['idx_chg'].shift(1).rolling(20).std()
        g['ma_spread'] = g['idx_ma5'] / g['idx_ma20'].replace(0, np.nan) - 1
    ...
    feature_cols = [
        ...
        'idx_pct_5d', 'idx_pct_20d',
        'idx_vol_10d', 'idx_vol_20d', 'ma_spread',
        # MACD/RSI/ADX
    ...
    rank_features = [
        ...
        # V8.4 指数相对排名特征
        'idx_pct_5d', 'idx_pct_20d', 'idx_vol_20d', 'ma_spread',
    ]
