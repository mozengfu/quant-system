#!/usr/bin/env python3
"""
ML选股模型训练 V6.6 — V6.3基础 + 五组新因子

相较 V6.5 新增五组因子:
  - 大宗交易溢价（成交额/溢价率/机构买家占比）
  - 板块资金流向（行业特大单净流入/板块涨跌幅）
  - 北向资金个股持仓（持仓比例/变化）
  - 业绩预告（预告类型/净利润变动）
  - 保持 V6.5 全部 105 个有效特征 + 横截面排名
"""
...
MODEL_PATH = os.path.join(DATA_DIR, 'ml_stock_model_v6_6.pkl')
FEATURE_CONFIG_PATH = os.path.join(DATA_DIR, 'feature_config_v6_6.json')
...
def load_data():
    ...
    try:
        block_trade = pd.read_sql("""
            SELECT ts_code, trade_date, premium_rate, deal_amount, deal_volume, buyer
            FROM block_trade
            WHERE trade_date >= %s
        """, conn, params=(start_date,))
        if not block_trade.empty:
            block_trade['trade_date'] = pd.to_datetime(block_trade['trade_date'])
            for c in ['premium_rate', 'deal_amount', 'deal_volume']:
                block_trade[c] = pd.to_numeric(block_trade[c], errors='coerce').fillna(0)
            block_trade['buyer'] = block_trade['buyer'].fillna('')
    except Exception as e:
        logger.warning(f"大宗交易数据加载失败: {e}")
    logger.info(f"大宗交易: {len(block_trade):,} 条 ({block_trade['ts_code'].nunique() if not block_trade.empty else 0} 只)")
    ...
    try:
        ...
        if not sector_mf.empty:
            ...
            for c in ['net_amount', 'buy_elg_amount', 'sell_elg_amount', 'pct_change']:
                sector_mf[c] = pd.to_numeric(sector_mf[c], errors='coerce').fillna(0)
    except Exception as e:
        logger.warning(f"板块资金流向加载失败: {e}")
    logger.info(f"板块资金流向: {len(sector_mf):,} 条 ({sector_mf['sector_name'].nunique() if not sector_mf.empty else 0} 板块)")

    # 北向个股持仓
    hsgt_hold = pd.DataFrame()
    try:
        hsgt_hold = pd.read_sql("""
            SELECT ts_code, trade_date, hold_ratio, hold_shares, hold_mv, `rank`
            FROM hsgt_hold_stock
            WHERE trade_date >= %s AND ts_code != '000000.NORTH'
        """, conn, params=(start_date,))
        if not hsgt_hold.empty:
            hsgt_hold['trade_date'] = pd.to_datetime(hsgt_hold['trade_date'])
            for c in ['hold_ratio', 'hold_shares', 'hold_mv']:
                hsgt_hold[c] = pd.to_numeric(hsgt_hold[c], errors='coerce').fillna(0)
    except Exception as e:
        logger.warning(f"北向持仓数据加载失败: {e}")
    logger.info(f"北向持仓: {len(hsgt_hold):,} 条 ({hsgt_hold['ts_code'].nunique() if not hsgt_hold.empty else 0} 只)")

    # 业绩预告
    stock_forecast = pd.DataFrame()
    try:
        stock_forecast = pd.read_sql("""
            SELECT ts_code, end_date, report_date, forecast_type, net_profit_min, net_profit_max
            FROM stock_forecast
        """, conn)
        if not stock_forecast.empty:
            ...
            for c in ['net_profit_min', 'net_profit_max']:
                stock_forecast[c] = pd.to_numeric(stock_forecast[c], errors='coerce')
    except Exception as e:
        logger.warning(f"业绩预告数据加载失败: {e}")
    ...
    logger.info(f"行情: {len(daily):,} 行, {daily['ts_code'].nunique()} 股 | "
                ...
                f"大宗交易: {len(block_trade):,} | 板块资金: {len(sector_mf):,} | "
                f"北向持仓: {len(hsgt_hold):,} | 业绩预告: {len(stock_forecast):,} | "
    ...
    return (daily, moneyflow, index_data, fundamentals, stock_info, alpha_signals,
            ...
            block_trade, sector_mf, hsgt_hold, stock_forecast,
            min_date, max_date)
...
def build_features(daily, moneyflow, index_data, fundamentals, stock_info, alpha_signals,
                   ...
                   min_date, max_date):
    """
    V6.6 特征构建 — V6.5 全部特征 + 大宗交易/板块资金流向/北向资金/业绩预告
    """
    logger.info("构建特征 V6.6...")
    ...
    for ts_code, group in daily.groupby('ts_code'):
        ...
        # ====== V6.6 新增: 大宗交易溢价特征 ======
        bt_stock = block_trade[block_trade['ts_code'] == ts_code].copy()
        if not bt_stock.empty:
            bt_stock['trade_date'] = pd.to_datetime(bt_stock['trade_date'])
            bt_stock = bt_stock.sort_values('trade_date')
            # 用 shift(1) 前值，T 日只看 T-1 以前的数据
            bt_stock['premium_rate_shifted'] = bt_stock['premium_rate'].shift(1)
            bt_stock['deal_amount_shifted'] = bt_stock['deal_amount'].shift(1)

            g['bt_count_30d'] = bt_stock['premium_rate'].rolling(30, min_periods=1).count().shift(1).values[-1] if len(g) > 1 else 0
            g['bt_premium_avg_30d'] = bt_stock['premium_rate'].rolling(30, min_periods=1).mean().shift(1).values[-1] if len(g) > 1 else 0
            g['bt_amount_30d'] = np.log(bt_stock['deal_amount'].rolling(30, min_periods=1).sum().shift(1).values[-1] + 1) if len(g) > 1 else 0

            # 机构买家特征
            inst_keywords = ['机构', '证券', '资管', '基金']
            bt_stock['is_inst_buyer'] = bt_stock['buyer'].apply(lambda x: 1 if any(kw in str(x) for kw in inst_keywords) else 0)
            g['bt_inst_buyer_ratio_30d'] = bt_stock['is_inst_buyer'].rolling(30, min_periods=1).mean().shift(1).values[-1] if len(g) > 1 else 0
        else:
            g['bt_count_30d'] = 0
            g['bt_premium_avg_30d'] = 0.0
            g['bt_amount_30d'] = 0.0
            g['bt_inst_buyer_ratio_30d'] = 0.0

        # ====== V6.6 新增: 板块资金流向特征 ======
        if industry and not sector_mf.empty:
            smf = sector_mf[sector_mf['sector_name'] == industry].copy()
            if not smf.empty:
                smf['trade_date'] = pd.to_datetime(smf['trade_date'])
                smf = smf.sort_values('trade_date')
                smf['elg_net'] = smf['buy_elg_amount'] - smf['sell_elg_amount']
                smf = smf.set_index('trade_date')
                # 按股票交易日期对齐
                smf_aligned = smf.reindex(g['trade_date'].values)
                g['sector_elg_net_today'] = smf_aligned['elg_net'].shift(1).fillna(0).values
                g['sector_pct_today'] = smf_aligned['pct_change'].shift(1).fillna(0).values
                g['sector_elg_net_cum5'] = smf_aligned['elg_net'].shift(1).rolling(5, min_periods=1).sum().fillna(0).values
                g['sector_pct_5d'] = smf_aligned['pct_change'].shift(1).rolling(5, min_periods=1).sum().fillna(0).values
            else:
        ...
        # ====== V6.6 新增: 北向资金个股持仓特征 ======
        hsgt_stock = hsgt_hold[hsgt_hold['ts_code'] == ts_code].copy()
        if not hsgt_stock.empty:
            hsgt_stock['trade_date'] = pd.to_datetime(hsgt_stock['trade_date'])
            hsgt_stock = hsgt_stock.sort_values('trade_date').set_index('trade_date')
            hsgt_aligned = hsgt_stock.reindex(g['trade_date'].values)
            g['hsgt_has_hold'] = (hsgt_aligned['hold_ratio'].shift(1) > 0).astype(int).fillna(0).values
            g['hsgt_hold_ratio'] = hsgt_aligned['hold_ratio'].shift(1).fillna(0).values
            g['hsgt_ratio_chg_5d'] = (hsgt_aligned['hold_ratio'].shift(1) - hsgt_aligned['hold_ratio'].shift(6)).fillna(0).values
        else:
            g['hsgt_has_hold'] = 0
            g['hsgt_hold_ratio'] = 0.0
            g['hsgt_ratio_chg_5d'] = 0.0

        # ====== V6.6 新增: 业绩预告特征 ======
        fc = stock_forecast[stock_forecast['ts_code'] == ts_code].copy()
        if not fc.empty:
            fc = fc.sort_values('report_date')
            # 取最新的预告
            fc_latest = fc.iloc[-1]
            ftype = str(fc_latest.get('forecast_type', ''))
            type_map = {'预增': 2, '扭亏': 2, '略增': 1, '续盈': 1,
                        '略减': -1, '预减': -2, '首亏': -2, '续亏': -3}
            for i in range(len(g)):
                g.loc[g.index[i], 'forecast_type_code'] = type_map.get(ftype, 0)
                g.loc[g.index[i], 'forecast_is_positive'] = 1 if ftype in ('预增', '扭亏', '略增', '续盈') else 0
                g.loc[g.index[i], 'forecast_net_profit_max'] = safe_float(fc_latest.get('net_profit_max', 0))
                try:
                    report_dt = datetime.strptime(str(fc_latest['report_date'])[:10], '%Y-%m-%d')
                    t_date = datetime.strptime(g.index[i].strftime('%Y-%m-%d'), '%Y-%m-%d')
                    g.loc[g.index[i], 'forecast_days_since'] = (t_date - report_dt).days
                except:
                    g.loc[g.index[i], 'forecast_days_since'] = 30
        else:
    ...
    feature_cols = [
        ...
        # ---- V6.6 新增: 大宗交易溢价 ----
        'bt_count_30d', 'bt_premium_avg_30d', 'bt_amount_30d', 'bt_inst_buyer_ratio_30d',
        # ---- V6.6 新增: 板块资金流向 ----
        'sector_elg_net_today', 'sector_pct_today', 'sector_elg_net_cum5', 'sector_pct_5d',
        # ---- V6.6 新增: 北向资金 ----
        'hsgt_has_hold', 'hsgt_hold_ratio', 'hsgt_ratio_chg_5d',
        # ---- V6.6 新增: 业绩预告 ----
        'forecast_type_code', 'forecast_is_positive', 'forecast_net_profit_max', 'forecast_days_since',
    ]
    ...
    rank_features = [
        ...
        # V6.6 新增排名特征
        'bt_count_30d', 'bt_premium_avg_30d', 'bt_amount_30d',
...
def train_ensemble(df, feature_cols, n_models=5):
    ...
    for i in range(n_models):
        ...
        params['feature_fraction'] = 0.75 + (i % 3) * 0.05  # 0.75, 0.80, 0.85, 0.75, 0.80

        model = lgb.train(
            params, base_ds, num_boost_round=300,
...
def main():
    ...
    bundle = {
        ...
        'label': 'lambda_rank_alpha_5d_vs_industry',
