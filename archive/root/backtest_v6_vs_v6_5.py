...
def main():
    logger.info("=" * 60)
    logger.info("V6 vs V6.5 ML选股模型对比验证")
    logger.info("=" * 60)

    # 加载两套模型
    v6 = load_model("v6")
    v65 = load_model("v6.5")
    if v6 is None or v65 is None:
        logger.error("模型加载失败！")
        return
    ...
    def predict_single_day(ts_codes, as_of_date):
        ...
        # V6.5
        feat65 = _build_features_for_stocks_v6_3(conn, ts_codes, as_of_date=as_of_date)
        if feat65.empty:
            return pred6, {}
        pred65 = dict(zip(feat65['ts_code'], _ensemble_predict(feat65, v65)))
    ...
    for i, date in enumerate(dates):
        ...
        try:
            pred6, pred65 = predict_single_day(all_codes, as_of_date=date)
            if not pred6 or not pred65:
                continue

            common = set(pred6.keys()) & set(pred65.keys())
            if len(common) < 30:
                continue
            ...
            data = pd.DataFrame({
                ...
                'v65_pred': [pred65[c] for c in common2],
            })

            # Rank IC
            ic_v6 = spearmanr(data['future_ret_5d'], data['v6_pred'])[0]
            ic_v65 = spearmanr(data['future_ret_5d'], data['v65_pred'])[0]

            # Top/Bottom spread
            n_top = max(10, int(len(data) * 0.20))
            for label, pred_col in [('v6', 'v6_pred'), ('v65', 'v65_pred')]:
                ...
                records.append({
                    'date': str(date),
                    'model': label,
                    'rank_ic': ic_v6 if label == 'v6' else ic_v65,
                    'top20_avg': top_avg,
    ...
    v65_ic = compare['rank_ic']['v65'].dropna()
    common_dates = v6_ic.index.intersection(v65_ic.index)
    v6_ic = v6_ic[common_dates]
    v65_ic = v65_ic[common_dates]
    delta_ic = v65_ic - v6_ic
    logger.info(f"\n--- 配对对比 (共同交易日: {len(common_dates)}) ---")
    logger.info(f"  V6   Rank IC 均值: {v6_ic.mean():.4f}")
    logger.info(f"  V6.5 Rank IC 均值: {v65_ic.mean():.4f}")
    logger.info(f"  Delta IC: {delta_ic.mean():.4f} ({delta_ic.mean()/v6_ic.mean()*100:+.1f}%)")
    logger.info(f"  V6.5 胜出天数: {(delta_ic>0).sum()}/{len(delta_ic)} ({(delta_ic>0).mean()*100:.1f}%)")

    # Spread 对比
    v6_spread = compare['spread_bps']['v6'][common_dates]
    v65_spread = compare['spread_bps']['v65'][common_dates]
    logger.info(f"  V6   Spread 均值: {v6_spread.mean():.2f}bp")
    logger.info(f"  V6.5 Spread 均值: {v65_spread.mean():.2f}bp")

    # Top10 对比
    v6_top = compare['top20_avg']['v6'][common_dates]
    v65_top = compare['top20_avg']['v65'][common_dates]
    logger.info(f"  V6   Top20 平均收益: {v6_top.mean():.2f}%")
    logger.info(f"  V6.5 Top20 平均收益: {v65_top.mean():.2f}%")

    # 保存详细结果
    out_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data', 'v6_vs_v6_5_comparison.json')
    summary = {
        ...
        'v6_5': {
            'mean_rank_ic': round(float(v65_ic.mean()), 4),
            'mean_daily_ic': round(float(v65_ic.mean()), 4),
            'ic_ir': round(float(v65_ic.mean() / (v65_ic.std() + 1e-9)), 2),
            'mean_spread_bps': round(float(v65_spread.mean()), 2),
            'mean_top20_ret': round(float(v65_top.mean()), 2),
        },
        'improvement': {
