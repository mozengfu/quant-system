...
gen_bundle = joblib.load('data/ml_stock_model.pkl')
...
for col, val in gen_bundle['global_medians'].items():
...
# 向量化预测
bear_probs = bear_bundle['model'].predict_proba(bear[bear_bundle['feature_cols']].values)[:, 1]
bear['prob'] = bear_probs
bear_high = bear[bear['prob'] > 0.55]
print(f"大盘跌 (>0.5%) 且高概率 (>55%) 样本: {len(bear_high):,} 个")
if len(bear_high) > 0:
    print(f"  实际胜率 (5天后涨>3%): {bear_high['label'].mean()*100:.1f}% (基线: {bear['label'].mean()*100:.1f}%)")
    print(f"  逆市高票示例:")
    for _, r in bear_high.nlargest(3, 'prob').iterrows():
        print(f"    {r['ts_code']} {r['trade_date']} 概率:{r['prob']:.1%} 实际:{r['future_ret']:.1%}")
else:
    print("  (无高概率样本，逆市模型太保守？)")
