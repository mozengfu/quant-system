...
def backtest_stock_v4(code, market="sz", start_date="", end_date=""):
    try:
        ...
        ts_code = "{}.{}".format(code, "SZ" if market == "sz" else "SH")
        if not end_date:
        ...
        base_stop = 0
        trailing_stop = 0
        shares_sold_t1 = False
        shares_sold_t2 = False
        peak_pnl = 0
        hold_days = 0
        ...
        for i in range(25, len(closes)):
            ...
            if shares_held == 0:
                ...
                if ma5 > ma10 > ma20 and price > ma5 and vol_ratio > 1.0:
                    buy = True
                    reason = f"均线多头({ma5:.2f}>{ma10:.2f}>{ma20:.2f}) 量比{vol_ratio:.2f}"
                elif pct_chg > 3.0 and vol_ratio > 1.0:
                    buy = True
                    reason = f"涨幅{pct_chg:.2f}% 量比{vol_ratio:.2f}"
                if buy:
                    ...
                    if qty > 0:
                        ...
                        hold_days = 0
                        trades.append(
                            {"日期": dates[i], "操作": "买入", "价格": round(price, 2), "数量": qty, "原因": reason}
                        )
            else:
                hold_days += 1
                ...
                if pnl_pct > peak_pnl:
                    peak_pnl = pnl_pct
                if peak_pnl >= 15:
                    trailing_stop = round(entry_price * 1.10, 2)
                elif peak_pnl >= 10:
                    trailing_stop = round(entry_price * 1.05, 2)
                elif peak_pnl >= 5:
                    trailing_stop = round(entry_price * 1.00, 2)
                else:
                    trailing_stop = base_stop
                sold = False
                    ...
                    trades.append(
                        {
                            ...
                            "数量": shares_held,
                            "盈亏": round(
                                (price - entry_price) * shares_held
                                - shares_held * entry_price * (BUY_COMMISSION + SELL_COMMISSION),
                                2,
                            ),
                            "收益率": f"{pnl_pct:.2f}%",
                            "原因": reason,
                ...
                if not sold and shares_held > 0 and (price >= entry_price * 1.18 or pct_chg >= 17.5):
                    sell("清仓", pnl_pct, "止盈+18%%")
                    continue
                if (
                    not sold
                    and shares_held > 0
                    and not shares_sold_t2
                    and (price >= entry_price * 1.10 or pct_chg >= 9.5)
                ):
                    q = int(shares_held / 2)
                    if q > 0:
                        proceeds = q * price * (1 - SELL_COMMISSION)
                        cash += proceeds
                        shares_held -= q
                        shares_sold_t2 = True
                        trades.append(
                            {
                                ...
                                "数量": q,
                                "盈亏": round(
                                    (price - entry_price) * q - q * entry_price * (BUY_COMMISSION + SELL_COMMISSION), 2
                                ),
                                "收益率": f"{pnl_pct:.2f}%",
                                "原因": "止盈+10%%",
                            }
                ...
                if (
                    not sold
                    and shares_held > 0
                    and not shares_sold_t1
                    and (price >= entry_price * 1.06 or pct_chg >= 5.5)
                ):
                    q = int(shares_held / 2)
                    if q > 0:
                        proceeds = q * price * (1 - SELL_COMMISSION)
                ...
                if not sold and shares_held > 0 and (price <= trailing_stop or hold_days >= MAX_HOLD_DAYS):
                    r = f"止损(={trailing_stop:.2f})" if price <= trailing_stop else f"超时{MAX_HOLD_DAYS}天清仓"
                    sell("卖出", pnl_pct, r)
        ...
        return {
            ...
            "策略": "V4 规则回测",
            "初始资金": "10万元",
            "最终市值": f"{final_value:.2f}元",
            "总收益率": f"{total_return:.2f}%",
            "交易次数": len(closed),
            "盈利次数": wins,
            "亏损次数": losses,
            "胜率": "%.1f%%" % (wins / max(len(closed), 1) * 100),
            "交易记录": trades[-30:],
        }
    except Exception:
...
def _run_strategy_backtest(code, market, start_date, end_date, strategy_name, condition_fn):
    ...
    try:
        ...
        return {
            ...
            "回测区间": f"{start_date} ~ {end_date}",
            "触发次数": len(signals),
            "5日胜率": f"{win5 / len(signals) * 100:.1f}% ({win5}/{len(signals)})",
            "10日胜率": f"{win10 / len(signals) * 100:.1f}% ({win10}/{len(signals)})",
            "5日平均收益": f"{avg5:.2f}%",
            "10日平均收益": f"{avg10:.2f}%",
            "最佳3次": top3,
            "最差3次": worst3,
...
def backtest_bottom(code, market="sz", start_date="", end_date=""):
    ...
    return _run_strategy_backtest(
        code, market, start_date, end_date, "底部起步", lambda d: -5 <= d["pct_chg"] <= 10 and d["vr"] > 1.0
    )
...
def backtest_strong(code, market="sz", start_date="", end_date=""):
    ...
    return _run_strategy_backtest(
        code, market, start_date, end_date, "强势活跃", lambda d: d["pct_chg"] > 3 and d["vr"] > 2.0 and d["tr"] > 5.0
    )
...
def backtest_combo(code, market="sz", start_date="", end_date=""):
    ...
    def cond(d):
        if d["close"] <= 5 or d["tr"] <= 1.5:
            return False
        if not (1 < d["pct_chg"] < 9.5):
            return False
