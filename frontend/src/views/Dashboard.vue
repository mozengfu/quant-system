  ...
  <div class="dashboard">
    ...
    <div class="kpi-row">
      ...
      <div class="kpi">
        ...
        <div :class="['kpi-v', pnlCls(store.performance.total_return)]">{{ perfReturnLabel }}</div>
        <div class="kpi-s">夏普 {{ shapely(store.performance.sharpe) }} <span class="dim">胜率 {{ winRateLabel }}</span></div>
      </div>
      <div class="kpi">
        <div class="kpi-h">
...
const perfReturnLabel = computed(() => {
  const v = store.performance.total_return
  if (v == null) return '-'
  return (v >= 0 ? '+' : '') + v.toFixed(2) + '%'
})

const winRateLabel = computed(() => {
  const v = store.performance.win_rate
  if (v == null) return '-'
